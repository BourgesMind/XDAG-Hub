...
import { ClientAxios } from "xdag/typescript/rpc/ClientAxios";
// 移除这行导入
	...
	async getTransactionBlock(input: { digest: string, pageNumber?: number }): Promise<XDagTransactionBlockResponse> {
		...
		if (response?.address && response?.hash) {
			if (response.state === 'Pending') {
				// 使用 localStorage 替代 Browser.storage.local
				if (typeof window !== 'undefined') {
					localStorage.setItem('pendingBlockAddress', response.address);
				}
			} else {
				// 使用 localStorage 替代 Browser.storage.local
				if (typeof window !== 'undefined') {
					const pendingBlockAddress = localStorage.getItem('pendingBlockAddress');
					if (pendingBlockAddress === response?.address) {
						localStorage.removeItem('pendingBlockAddress');
					}
				}
			}
			return response
